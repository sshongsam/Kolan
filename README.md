...
- **사용 가이드** — 5단계 학습법과 FAQ를 담은 안내 화면 (가이드 탭)
...
| `dist/kolan-study-pack.pdf` | **전체 학습자료 PDF 완성본** — 앱의 📄 PDF 버튼과 동일한 내용, 바로 배포 가능 |
| `scripts/build.mjs` | 배포판 빌드 스크립트 (콘텐츠 인라인 + PDF 생성·내장) |
| `i18n/vi.html` `mn.html` `zh.html` | 베트남어·몽골어·중국어 캠퍼스 생활 학습 앱 (웹 배포: `/vi/` `/mn/` `/zh/`) |
| `police/` | 경찰행정학과 실종아동찾기 시뮬레이션 실습 시스템 8종 + 허브 (웹 배포: `/police/`) |

## 변경 이력 · Changelog

- **v1.5.0** — 경찰행정학과 실종아동찾기 프로젝트 시뮬레이션 8종 배포 (`/police/`): K-FINDER V3 통합훈련, 실시간 관제 포털, 신고접수, 제보분석, 격자수색, 수사지휘, 가족지원 + 실습 허브 페이지
- **v1.4.3** — 다국어 확장: 베트남어(/vi/)·몽골어(/mn/)·중국어(/zh/) 캠퍼스 생활 학습 앱 배포(교수자 제공 콘텐츠, `i18n/`), 메인 앱 홈에 언어 선택 카드 추가
- **v1.4.2** — 인앱 브라우저(카카오톡 등) 듣기 미지원 대응: 감지 시 안내 배너 + "Chrome/Safari에서 열기" 버튼(카카오톡 전용 탈출 링크·안드로이드 인텐트 지원), iOS 간헐 무음 버그 보강
- **v1.4.1** — 📄 PDF 버튼이 완성된 PDF 파일을 **바로 다운로드**하도록 개선 (인쇄 대화상자가 차단되는 웹뷰·인앱 브라우저 대응, PDF를 앱 파일에 내장). `scripts/build.mjs` 빌드 스크립트 추가
- **v1.4.0** — 📄 PDF 학습자료 다운로드 버튼 추가 (네비게이션): 10개 모듈 50문장 전체를 인쇄용 문서로 생성, 문장별 따라 쓰기 줄 + 모듈별 메모 칸 포함, 브라우저 "PDF로 저장"으로 다운로드
- **v1.3.1** — 앱 사용 가이드 화면 추가 (📖 가이드 탭 + 홈 화면 바로가기 버튼, 5단계 학습법·진도 안내·단축키·소리 문제 해결)
